...
if (!class_exists('Yab_Tour_Carousel_Renderer')) {
    ...
    class Yab_Tour_Carousel_Renderer extends Yab_Abstract_Banner_Renderer {
        ...
        /**
         * Clones slides for loop mode based on the final, detailed scenario.
         *
        ...
        private function get_cloned_slides_for_loop(array $tours, int $slides_per_view): array {
            ...
            if ($tour_count === 1) {
                $clones_needed = $slides_per_view;
                $cloned_tours = $tours;
                for ($i = 0; $i < $clones_needed; $i++) {
                    $cloned_tours[] = $tours[0];
                }
                return $cloned_tours;
            }

            // Case 2: More than one tour, but still less than or equal to slidesPerView.
            if ($tour_count > 1 && $tour_count <= $slides_per_view) {
                $cloned_tours = $tours;
                // Keep duplicating the sequence until the total count is at least 2 * slidesPerView
                while (count($cloned_tours) < $slides_per_view * 2) {
                    $cloned_tours = array_merge($cloned_tours, $tours);
                }
                return $cloned_tours;
            }

            // Default case: If tour_count > slidesPerView, no cloning is needed.
            return $tours;
        }
        ...
        public function render(): string {
            ...
            <style>
                ...
                /* *** FIX: Improved Skeleton Styles *** */
                .yab-tour-card-skeleton {
                    width: 295px; height: 375px; background-color: #fff; border-radius: 14px; padding: 9px;
                    display: flex; flex-direction: column; gap: 9px; overflow: hidden; border: 1px solid #f0f0f0;
                }
                .yab-skeleton-image { width: 100%; height: 204px; background-color: #f0f0f0; border-radius: 14px; }
                .yab-skeleton-text-container { padding: 14px 5px 5px 5px; display: flex; flex-direction: column; gap: 10px; flex-grow: 1; }
                .yab-skeleton-text { background-color: #f0f0f0; border-radius: 4px; }
                            ...
                            <?php foreach ($selected_tours as $tour_id) : ?>
                                <div class="swiper-slide" data-tour-id="<?php echo esc_attr($tour_id); ?>">
                                    <div class="yab-tour-card-skeleton yab-skeleton-loader">
                                        ...
                                        <div class="yab-skeleton-text-container">
                                            <div class="yab-skeleton-text" style="width: 80%; height: 20px;"></div>
                                            <div class="yab-skeleton-text" style="width: 40%; height: 16px;"></div>
                                            <div style="display: flex; justify-content: space-between; align-items: center; margin-top: auto; padding-bottom: 5px;">
                                                <div class="yab-skeleton-text" style="width: 40%; height: 20px;"></div>
                                                <div class="yab-skeleton-text" style="width: 30%; height: 16px;"></div>
                                            </div>
