<?php
// tappersia/includes/BannerTypes/ContentHtmlSidebarBanner/ContentHtmlSidebarBanner.php

if (!class_exists('Yab_Content_Html_Sidebar_Banner')) :
class Yab_Content_Html_Sidebar_Banner {
    ...
    public function save($banner_data) {
        ...
        if (is_wp_error($result)) {
            ...
            update_post_meta($result, '_yab_banner_type', 'content-html-sidebar-banner');
    ...
    private function check_for_banner_conflict($displayOn, $current_banner_id) {
        ...
        $args = [
            ...
            'meta_query'   => [
                ...
                ['key' => '_yab_banner_type', 'value' => 'content-html-sidebar-banner', 'compare' => '=']
            ],
        ...
        foreach ($other_banners_query->posts as $banner_post) {
            ...
            if (!empty(array_intersect($post_ids, $other_post_ids))) {
                return ['has_conflict' => true, 'message' => 'A Content HTML Sidebar banner is already assigned to one of the selected posts.'];
            }
            ...
            if (!empty(array_intersect($page_ids, $other_page_ids))) {
                 return ['has_conflict' => true, 'message' => 'A Content HTML Sidebar banner is already assigned to one of the selected pages.'];
            }
        }
        return $conflict;
    }
    ...
    private function sanitize_banner_data($data) {
        ...
        $sanitized['type'] = isset($data['type']) ? sanitize_text_field($data['type']) : 'content-html-sidebar-banner';

        if (isset($data['content_html_sidebar']) && is_array($data['content_html_sidebar'])) {
            $sanitized['content_html_sidebar']['html'] = $data['content_html_sidebar']['html'];
        } else {
            $sanitized['content_html_sidebar']['html'] = '';
        }
